export const API_CONFIG = {
	...
	VERIFICATION_BASE_URL:
		import.meta.env.VITE_VERIFICATION_API_BASE_URL ||
		"http://localhost:8000",
	PROCTORING_BASE_URL:
		import.meta.env.VITE_PROCTORING_API_BASE_URL || "http://localhost:8889",
	TIMEOUT: 30000,
...
export const API_ENDPOINTS = {
	...
	ASSESSMENT_UPDATE_QUESTION: (id: number, questionId: number) =>
		`/api/v1/assessments/${id}/questions/${questionId}`,
	ASSESSMENT_BULK_ADD_QUESTIONS: (id: number) =>
		`/api/v1/assessments/${id}/questions/batch`,
	ASSESSMENT_BULK_REMOVE_QUESTIONS: (id: number) =>
		`/api/v1/assessments/${id}/questions/batch`,
	ASSESSMENT_BULK_UPDATE_QUESTIONS: (id: number) =>
		`/api/v1/assessments/${id}/questions/batch`,
	ASSESSMENT_REORDER_QUESTIONS: (id: number) =>
	...
	ASSESSMENT_RESULTS_EXPORT: (id: number) =>
		`/api/v1/assessments/${id}/results/export`,
	...
	QUESTION_BANK_QUESTIONS: (id: number) =>
		`/api/v1/question-banks/${id}/questions`,
	QUESTION_BANK_ADD_QUESTIONS: (id: number) =>
		`/api/v1/question-banks/${id}/questions`,
	QUESTION_BANK_REMOVE_QUESTIONS: (id: number) =>
		`/api/v1/question-banks/${id}/questions`,
	...
	ATTEMPT_TIME_REMAINING: (id: number) =>
		`/api/v1/attempts/${id}/time-remaining`,
	ATTEMPT_CAN_START: (assessmentId: number) =>
		`/api/v1/attempts/can-start/${assessmentId}`,
	ATTEMPT_CURRENT: (assessmentId: number) =>
		`/api/v1/attempts/current/${assessmentId}`,
	ATTEMPT_COUNT: (assessmentId: number) =>
		`/api/v1/attempts/count/${assessmentId}`,
	ATTEMPT_BY_STUDENT: (studentId: string | number) =>
		`/api/v1/attempts/student/${studentId}`,
	ATTEMPT_BY_ASSESSMENT: (assessmentId: number) =>
		`/api/v1/attempts/assessment/${assessmentId}`,

	// Grading
	GRADING_ANSWER: (answerId: number) => `/api/v1/grading/answers/${answerId}`,
	GRADING_ATTEMPT: (attemptId: number) =>
		`/api/v1/grading/attempts/${attemptId}`,
	GRADING_AUTO: (attemptId: number) =>
	...
	DASHBOARD_PERFORMANCE_BY_SUBJECT:
		"/api/v1/dashboard/performance-by-subject",

	// Student Panel
	STUDENT_STATS: "/api/v1/students/me/stats",
	STUDENT_ASSESSMENTS: "/api/v1/students/me/assessments",
	STUDENT_ASSESSMENT_DETAIL: (id: number) =>
		`/api/v1/students/me/assessments/${id}`,
	...
	GROUP_MEMBER: (id: number, userId: string) =>
	...
	ASSESSMENT_GROUPS: (assessmentId: number) =>
		`/api/v1/assessments/${assessmentId}/groups`,
};
