...
const AssessmentResults: React.FC = () => {
	...
	const renderAnswerFeedback = (answer: StudentAnswer, questionNumber: number) => {
		...
		const renderStudentAnswer = () => {
			if (!answer.answer) return <Text type="secondary">{t('assessmentResults.question.notAnswered')}</Text>;
				...
				case 'true_false':
					return (
						<Tag color={answer.answer === true ? 'blue' : 'orange'} style={{ fontSize: '14px', padding: '4px 12px' }}>
							{answer.answer === true ? (questionContent.true_label || t('assessmentResults.question.true')) : (questionContent.false_label || t('assessmentResults.question.false'))}
						</Tag>
					);
							...
							{typeof answer.answer === 'object'
								? Object.entries(answer.answer).map(([key, value]: [string, any]) => (
									<div key={key} style={{ marginBottom: '8px' }}>
										<Text strong>{t('assessmentResults.question.blank', { key })}:</Text> <Tag>{value}</Tag>
									</div>
								))
								: answer.answer}
						</div>
		...
		const renderCorrectAnswer = () => {
			switch (questionType) {
				...
				case 'true_false':
					...
					return (
						<Tag color="success" style={{ fontSize: '14px', padding: '4px 12px' }}>
							{correctBool === true ? (questionContent.true_label || t('assessmentResults.question.true')) : (questionContent.false_label || t('assessmentResults.question.false'))}
						</Tag>
					);
				...
				case 'essay':
					...
					return <Text type="secondary">{t('assessmentResults.question.manualGrading')}</Text>;
						...
						return (
							<div style={{ padding: '8px' }}>
								{Object.entries(questionContent.blanks).map(([key, blank]: [string, any]) => (
									<div key={key} style={{ marginBottom: '8px' }}>
										<Text strong>{t('assessmentResults.question.blank', { key })}</Text>{' '}
		...
		return (
			<Panel
				header={
					<Space>
						...
						<Text strong>{t('assessmentResults.questionN', { n: questionNumber })}</Text>
						<Tag color="blue">
							{t(`assessmentResults.questionType.${questionType}`, { defaultValue: questionType })}
						</Tag>
						{answer.score !== undefined && answer.max_score !== undefined && (
							<Text type="secondary">
								{t('assessmentResults.question.points', { score: answer.score, max: answer.max_score })}
							</Text>
						)}
					</Space>
				}
			...
			>
				<Space direction="vertical" style={{ width: '100%' }} size="middle">
					{/* Question Text */}
					<div>
						<Text strong style={{ fontSize: '16px' }}>{t('assessmentResults.question.text')}</Text>
				...
				<Space direction="vertical" size="large" style={{ width: '100%' }}>
					<div style={{ textAlign: 'center' }}>
						...
						<Title level={2} style={{ margin: 0 }}>
							{isPendingGrading ? t('assessmentResults.pendingGrading') : passed ? t('assessmentResults.congratulations') : t('assessmentResults.testCompleted')}
						</Title>
					...
					{isPendingGrading && ungradedCount > 0 && (
						<Alert
							message={t('assessmentResults.pendingGradingAlert', { ungraded: ungradedCount, total: totalCount })}
							description={t('assessmentResults.pendingGradingDescription', { count: ungradedCount })}
							type="warning"
							showIcon
							icon={<HourglassOutlined />}
					...
					<Row gutter={[16, 16]}>
						<Col xs={24} sm={12} md={6}>
							<Card>
								<Statistic
									title={isPendingGrading ? t('assessmentResults.scoreTempLabel') : t('assessmentResults.scoreLabel')}
									value={score}
									precision={1}
									suffix={`/ ${maxScore}`}
						...
						<Col xs={24} sm={12} md={6}>
							<Card>
								<Statistic
									...
									value={isPendingGrading ? t('assessmentResults.statusGrading') : passed ? t('assessmentResults.statusPassed') : t('assessmentResults.statusFailed')}
								...
								/>
								<Text type="secondary">
									{t('assessmentResults.passingScore', { score: attempt.assessment?.passing_score })}
								</Text>
							</Card>
						</Col>
						<Col xs={24} sm={12} md={6}>
						...
						<Col xs={24} sm={12} md={6}>
							<Card>
								...
								/>
								<Text type="secondary">
									{t('assessmentResults.timeLimit', { duration: attempt.assessment?.duration })}
								</Text>
							</Card>
						</Col>
					</Row>
					...
					{!isPendingGrading && (
						<Alert
							message={passed ? t('assessmentResults.passedAlert') : t('assessmentResults.failedAlert')}
							description={
								passed
									? t('assessmentResults.passedDescription', { percent: percentage.toFixed(1), passing: attempt.assessment?.passing_score })
									: `${t('assessmentResults.failedDescription', { percent: percentage.toFixed(1), passing: attempt.assessment?.passing_score })} ${attempt.assessment?.max_attempts && attempt.assessment.max_attempts > 1
						...
						<Space>
							...
							{isPendingGrading && ungradedCount > 0 && (
								<Tag color="warning" icon={<HourglassOutlined />}>
									{t('assessmentResults.gradedProgress', { graded: gradedCount, total: totalCount })}
								</Tag>
							)}
						</Space>
					}
						...
						columns={[
							{
								...
								key: 'question_number',
								render: (_, record, index) => t('assessmentResults.questionN', { n: index + 1 }),
								width: 100,
							},
							{
								...
								render: (_, record) => (
									<Space>
										...
										{record.partial_credit && (
											<Tag color="warning">{t('assessmentResults.partialCredit')}</Tag>
										)}
									</Space>
								),
								width: 200,
