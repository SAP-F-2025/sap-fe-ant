...
const StudentAssessmentList: React.FC = () => {
  ...
  const fetchAssessments = async () => {
    ...
    try {
      ...
      showError(t('assessmentList.loadError') || 'Failed to load assessments');
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns: ColumnsType<Assessment> = [
    {
      ...
      render: (text, record) => (
        <Space direction="vertical" size={0}>
          <Typography.Link
            strong
            onClick={() => navigate(`/student/manage-assessments/${record.id}`)}
          >
            {text}
          </Typography.Link>
          {record.description && (
        ...
        <Space size="small" style={{ display: 'flex' }}>
          ...
          <Tooltip title={t('assessmentList.edit')}>
            <Button
              type="text"
              icon={<EditOutlined />}
              onClick={() => navigate(`/student/manage-assessments/${record.id}/edit`)}
            />
          </Tooltip>
