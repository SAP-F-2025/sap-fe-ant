...
const GroupList: React.FC = () => {
	...
	const getRoleBadge = (role?: string) => {
		const roleMap: Record<string, { color: string; label: string }> = {
			[GroupMemberRole.Owner]: { color: 'gold', label: t('groups.role.owner') },
			[GroupMemberRole.CoOwner]: { color: 'blue', label: t('groups.role.coOwner') },
			[GroupMemberRole.Member]: { color: 'default', label: t('groups.role.member') },
		};
		if (!role) return null;
	...
	const getTypeBadge = (type: string) => {
		const typeMap: Record<string, { color: string; label: string }> = {
			...
			'study-group': { color: 'cyan', label: t('groups.type.studyGroup') },
		};
				...
				<Space size="small" style={{ display: 'flex' }}>
					...
					{record.can_delete && (
						<Popconfirm
							...
							description={t('groups.deleteConfirm.content', { name: record.display_name || record.name })}
		...
		<Space direction="vertical" size="large" style={{ width: '100%' }}>
			...
			<Card bordered={false} style={{ ...elevation[1], borderRadius: 16, background: '#f5f5f5' }}>
				<Space direction="vertical" size={8} style={{ width: '100%' }}>
					<Text type="secondary" style={{ fontSize: 13, fontWeight: 500 }}>{t('groups.statistics.title')}</Text>
					<Row gutter={[12, 12]}>
						<Col xs={12} sm={6}>
							<Flex align="center" gap={8}>
								...
								<Space direction="vertical" size={0}>
									...
									<Text type="secondary" style={{ fontSize: 12 }}>{t('groups.stats.totalGroups')}</Text>
								</Space>
							</Flex>
						</Col>
						<Col xs={12} sm={6}>
							<Flex align="center" gap={8}>
								...
								<Space direction="vertical" size={0}>
									...
									<Text type="secondary" style={{ fontSize: 12 }}>{t('groups.stats.classGroups')}</Text>
								</Space>
							</Flex>
						</Col>
						<Col xs={12} sm={6}>
							<Flex align="center" gap={8}>
								...
								<Space direction="vertical" size={0}>
									...
									<Text type="secondary" style={{ fontSize: 12 }}>{t('groups.stats.studyGroups')}</Text>
								</Space>
							</Flex>
						</Col>
					</Row>
			...
			>
				...
				>
					...
					<Form.Item name="description" label={t('groups.form.description')}>
						<Input.TextArea rows={3} placeholder={t('groups.form.descriptionPlaceholder')} />
					</Form.Item>
