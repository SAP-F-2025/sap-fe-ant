...
const mockQuestionDistribution: QuestionDistribution[] = [
  { type: 'multiple_choice', name: 'Trắc nghiệm', count: 450, percentage: 41.7 },
...
class DashboardService {
  async getDashboardStats(period: number = 30): Promise<DashboardStats> {
    ...
    return apiService.get<DashboardStats>(API_ENDPOINTS.DASHBOARD_STATS, { period });
  }

  async getActivityTrends(period: 'week' | 'month' | 'year' = 'month'): Promise<ActivityTrend[]> {
    if (API_CONFIG.USE_MOCK) {
      await delay();
      return mockActivityTrends;
    }

    return apiService.get<ActivityTrend[]>(API_ENDPOINTS.DASHBOARD_ACTIVITY_TRENDS, { period });
  }
  ...
  async getRecentActivities(limit: number = 10): Promise<RecentActivity[]> {
    ...
    return apiService.get<RecentActivity[]>(API_ENDPOINTS.DASHBOARD_RECENT_ACTIVITIES, { limit });
  }
  ...
  async getQuestionDistribution(): Promise<QuestionDistribution[]> {
    ...
    return apiService.get<QuestionDistribution[]>(API_ENDPOINTS.DASHBOARD_QUESTION_DISTRIBUTION);
  }

  async getPerformanceBySubject(limit: number = 5): Promise<SubjectPerformance[]> {
    if (API_CONFIG.USE_MOCK) {
      await delay();
