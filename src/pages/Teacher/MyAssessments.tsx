	...
	const columns = [
		...
		},
		{
			...
			render: (status: AssessmentStatus) => {
				...
				> = {
					Draft: { color: 'default', text: t('myAssessments.status.draft'), icon: <EditOutlined /> },
					Active: { color: 'success', text: t('myAssessments.status.active'), icon: <CheckCircleOutlined /> },
					Expired: { color: 'warning', text: t('myAssessments.status.expired'), icon: <ClockCircleOutlined /> },
					Archived: { color: 'default', text: t('myAssessments.status.archived'), icon: <FileTextOutlined /> },
				};
		...
		},
		{
			...
			width: 100,
			align: 'center' as const,
			render: (duration: number) => <Text>{duration} {t('common.minutes')}</Text>,
		},
		{
				...
				<Space direction="vertical" style={{ width: '100%' }} size="middle">
					...
					<Space style={{ width: '100%', justifyContent: 'space-between' }}>
						<Space>
							...
							<Select
								...
								allowClear
							>
								<Select.Option value="Draft">{t('myAssessments.status.draft')}</Select.Option>
								<Select.Option value="Active">{t('myAssessments.status.active')}</Select.Option>
								<Select.Option value="Expired">{t('myAssessments.status.expired')}</Select.Option>
								<Select.Option value="Archived">{t('myAssessments.status.archived')}</Select.Option>
							</Select>
						</Space>
						<Button
							type="primary"
					...
					<Table
						...
						pagination={{
							...
							showTotal: (total) => t('myAssessments.totalAssessments', { count: total }),
						}}
						scroll={{ x: 1200 }}
						locale={{
