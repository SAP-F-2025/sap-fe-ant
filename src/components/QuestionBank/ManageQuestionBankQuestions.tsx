...
export const ManageQuestionBankQuestions: React.FC<Props> = ({
	...
	const difficultyLabels = {
		[DifficultyLevel.Easy]: t(
			"manageQuestionBankQuestions.difficulty.easy",
		),
		[DifficultyLevel.Medium]: t(
			"manageQuestionBankQuestions.difficulty.medium",
		),
		[DifficultyLevel.Hard]: t(
			"manageQuestionBankQuestions.difficulty.hard",
		),
	};

	const typeLabels = {
		[QuestionType.MultipleChoice]: t(
			"manageQuestionBankQuestions.type.multipleChoice",
		),
		[QuestionType.TrueFalse]: t(
			"manageQuestionBankQuestions.type.trueFalse",
		),
		[QuestionType.Essay]: t("manageQuestionBankQuestions.type.essay"),
		[QuestionType.FillBlank]: t(
			"manageQuestionBankQuestions.type.fillBlank",
		),
		...
		[QuestionType.ShortAnswer]: t(
			"manageQuestionBankQuestions.type.shortAnswer",
		),
	};
	...
	const [filterDifficulty, setFilterDifficulty] = useState<
		string | undefined
	>();
	const [bankPagination, setPagination] = useState({
		page: 1,
		size: 10,
		total: 0,
	});
	...
	const fetchQuestions = async () => {
		setLoading(true);
		try {
			const data = await questionBankService.getQuestionBankQuestions(
				bankId,
				{
	...
	return (
		<>
			<Card
				title={t("manageQuestionBankQuestions.title", {
					count: bankPagination.total,
				})}
				extra={
					<Space>
						{selectedRowKeys.length > 0 && (
							<Popconfirm
								title={t(
									"manageQuestionBankQuestions.removeConfirm.title",
								)}
								description={t(
									"manageQuestionBankQuestions.removeConfirm.description",
									{ count: selectedRowKeys.length },
								)}
								onConfirm={handleRemoveQuestions}
								okText={t(
									"manageQuestionBankQuestions.removeConfirm.ok",
								)}
								cancelText={t(
									"manageQuestionBankQuestions.removeConfirm.cancel",
								)}
							>
								<Button danger icon={<DeleteOutlined />}>
									{t(
										"manageQuestionBankQuestions.removeBtn",
										{ count: selectedRowKeys.length },
									)}
								</Button>
							</Popconfirm>
			...
			>
				...
				>
					...
					{questions.length > 0 && (
						<Alert
							message={t(
								"manageQuestionBankQuestions.filterInfo",
					...
					<Row gutter={[8, 8]}>
						<Col span={12}>
							<Input
								placeholder={t(
									"manageQuestionBankQuestions.searchPlaceholder",
								)}
						...
						<Col span={6}>
							<Select
								placeholder={t(
									"manageQuestionBankQuestions.filterByType",
								)}
								allowClear
								style={{ width: "100%" }}
								value={filterType}
						...
						<Col span={6}>
							<Select
								placeholder={t(
									"manageQuestionBankQuestions.filterByDifficulty",
								)}
								allowClear
								style={{ width: "100%" }}
					...
					<Space>
						...
						>
							{fetchingQuestions
								? t("manageQuestionBankQuestions.searching")
								: t("manageQuestionBankQuestions.filter")}
						</Button>
						{!fetchingQuestions &&
							availableQuestions.length > 0 && (
								<Text type="secondary">
									{t("manageQuestionBankQuestions.found", {
							...
							emptyText: (
								...
								>
									...
									<Text type="secondary">
										{questions.length > 0
											? t(
													"manageQuestionBankQuestions.allQuestionsAdded",
												)
											: t(
													"manageQuestionBankQuestions.noQuestionsFound",
												)}
									</Text>
									{questions.length === 0 && (
										<Text
										...
										>
											{t(
												"manageQuestionBankQuestions.tryChangeFilter",
											)}
										</Text>
									)}
								</Space>
