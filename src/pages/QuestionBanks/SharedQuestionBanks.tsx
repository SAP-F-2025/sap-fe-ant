...
const SharedQuestionBanks: React.FC = () => {
	...
	const permissionLabels = {
		[QuestionBankSharePermission.ViewOnly]: t(
			"sharedQuestionBanks.permission.viewOnly",
		),
		[QuestionBankSharePermission.CanEdit]: t(
			"sharedQuestionBanks.permission.canEdit",
		),
		[QuestionBankSharePermission.CanDelete]: t(
			"sharedQuestionBanks.permission.fullAccess",
		),
	};
	...
	const fetchSharedQuestionBanks = async () => {
		...
		try {
			const response =
				await questionBankService.getSharedQuestionBanks(filters);
		...
		},
		{
			...
			render: (_, record: any) => {
				if (record.is_owner || record.access_level === "owner") {
					return (
						<Tag color="gold">
							{t("sharedQuestionBanks.permission.owner")}
						</Tag>
					);
				}
				...
				{questionBanks.length === 0 && !loading ? (
					<Empty
						description={
							<Space direction="vertical" size="small">
								<Text type="secondary">
									{t("sharedQuestionBanks.empty.title")}
								</Text>
					...
					<Table
						...
						pagination={{
							...
							showTotal: (total) =>
								t("sharedQuestionBanks.showTotal", { total }),
							onChange: (page, size) =>
