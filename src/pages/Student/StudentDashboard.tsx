...
const StudentDashboard: React.FC = () => {
			...
			render: (passed: boolean, record: any) => {
				...
				return (
					<Tag
						...
						icon={passed ? <CheckCircleOutlined /> : <CloseIcon />}
					>
						{passed
	...
	const upcomingColumns = [
		...
		},
		{
			...
			render: (days: number) => {
				const color =
					days <= 1 ? "red" : days <= 3 ? "orange" : "green";
				return (
					<Tag color={color}>
						{days === 0
							? t("studentDashboard.upcoming.today")
							: t("studentDashboard.upcoming.daysRemaining", {
									count: days,
								})}
					</Tag>
				);
		...
		},
		{
			...
			render: (_: any, record: any) => (
				<Button
					...
					size="small"
					onClick={() =>
						navigate(`/student/assessments/${record.id}`)
					}
				>
	...
	return (
		<div style={{ padding: "24px" }}>
			...
			<Text type="secondary">
				{t("studentDashboard.welcomeBack", { name: user?.displayName })}
			</Text>
			...
			<Row gutter={[16, 16]} style={{ marginTop: "24px" }}>
				<Col xs={24} sm={12} lg={6}>
					<Card loading={isLoading}>
						<Statistic
							title={t(
								"studentDashboard.overview.assessmentsAvailable",
							)}
							value={
								stats?.overview.total_assessments_available || 0
							}
							prefix={<BookOutlined />}
				...
				<Col xs={24} sm={12} lg={6}>
					<Card loading={isLoading}>
						<Statistic
							...
							value={
								stats?.overview.total_assessments_completed || 0
							}
				...
				<Col xs={24} sm={12} lg={6}>
					<Card loading={isLoading}>
						<Statistic
							...
							value={
								stats?.overview.total_assessments_in_progress ||
								0
							}
			...
			<Row gutter={[16, 16]} style={{ marginTop: "16px" }}>
				<Col xs={24} sm={12} lg={6}>
					<Card loading={isLoading}>
						<Statistic
							title={t(
								"studentDashboard.performance.averageScore",
							)}
				...
				<Col xs={24} sm={12} lg={6}>
					<Card loading={isLoading}>
						<Statistic
							title={t(
								"studentDashboard.performance.highestScore",
							)}
				...
				<Col xs={24} sm={12} lg={6}>
					<Card loading={isLoading}>
						<Statistic
							title={t(
								"studentDashboard.performance.lowestScore",
							)}
			...
			>
				<Table
					...
					locale={{
						emptyText: t(
							"studentDashboard.recentAttempts.noAttempts",
						),
					}}
				/>
			</Card>
			...
			>
				<Table
					...
					pagination={false}
					locale={{
						emptyText: t("studentDashboard.upcoming.noUpcoming"),
					}}
				/>
			</Card>
			...
			{/* Quick Actions */}
			<Card
				title={t("studentDashboard.quickActions.title")}
				style={{ marginTop: "24px" }}
			>
