	...
	const handleStartAssessment = async (assessment: StudentAssessment) => {
		try {
			...
			if (assessment.has_active_attempt) {
				modal.confirm({
					title: t(
						"availableAssessments.modals.continueAttempt.title",
					),
					icon: <ExclamationCircleOutlined />,
					content: t(
						"availableAssessments.modals.continueAttempt.content",
					),
					okText: t(
						"availableAssessments.modals.continueAttempt.okText",
					),
					cancelText: t(
						"availableAssessments.modals.continueAttempt.cancelText",
					),
					onOk: async () => {
						try {
							const attempt =
								await studentService.getCurrentAttempt(
									assessment.id,
								);
							if (attempt) {
								// Check if attempt has expired
								const timeData =
									await studentService.getTimeRemaining(
										attempt.id,
									);
								if (timeData.data <= 0) {
									...
									modal.warning({
										title: t(
											"availableAssessments.modals.timeout.title",
										),
										content: t(
											"availableAssessments.modals.timeout.content",
										),
										onOk: async () => {
							...
							modal.error({
								title: t(
									"availableAssessments.modals.error.title",
								),
								content:
									error.message ||
									t(
										"availableAssessments.modals.error.cannotContinue",
									),
							});
						}
					},
			...
			const canStartResult = await studentService.canStartAssessment(
				assessment.id,
			);
			if (!canStartResult.can_start) {
				modal.error({
					...
					content:
						...
						t(
							"availableAssessments.modals.cannotStart.defaultContent",
						),
				});
				return;
			}
			...
			const assessmentDetail = await studentService.getAssessmentDetail(
				assessment.id,
			);
			...
			modal.error({
				...
				content:
					error.message ||
					t("availableAssessments.modals.error.defaultContent"),
			});
		}
	};
		...
		modal.confirm({
			...
			content: (
				<div>
					...
					<p>
						{t(
							"availableAssessments.modals.startAssessment.duration",
							{ duration: selectedAssessment.duration },
						)}
					</p>
					<p>
						{t(
							"availableAssessments.modals.startAssessment.attempts",
							{
					...
					<p>
						{t(
							"availableAssessments.modals.startAssessment.passingScore",
							{ score: selectedAssessment.passing_score },
						)}
					</p>
					<Alert
						message={t(
							"availableAssessments.modals.startAssessment.warning",
						)}
						type="warning"
						showIcon
						style={{ marginTop: 16 }}
			...
			cancelText: t(
				"availableAssessments.modals.startAssessment.cancelText",
			),
			onOk: async () => {
				try {
		...
		},
		{
			...
			render: (duration: number) => (
				<Space>
					<ClockCircleOutlined />
					<Text>
						{t("availableAssessments.minutes", { count: duration })}
					</Text>
				</Space>
			),
		},
		...
		},
		{
			...
			render: (date: string | undefined) => {
				if (!date)
					return (
						<Text type="secondary">
							{t("availableAssessments.noDueDate")}
						</Text>
					);
				...
				return (
					<Tag
						color={
							isOverdue ? "red" : isUrgent ? "orange" : "default"
						}
					>
				...
				if (record.has_active_attempt) {
					return (
						<Tag color="processing">
							{t("availableAssessments.status.inProgress")}
						</Tag>
					);
				}
				if (!record.can_start) {
					return (
						<Tag color="error">
							{t("availableAssessments.status.unavailable")}
						</Tag>
					);
				}
				return (
					<Tag color="success">
						{t("availableAssessments.status.ready")}
					</Tag>
				);
			},
		},
		...
		<>
			...
			<div style={{ padding: "24px" }}>
				<div style={{ marginBottom: "24px" }}>
					...
					<Text type="secondary">
						{t("availableAssessments.subtitle")}
					</Text>
				</div>
				...
				<Card>
					...
					>
						...
						<Input
							placeholder={t(
								"availableAssessments.searchPlaceholder",
							)}
