...
const AssessmentDetail: React.FC = () => {
	...
	const getStatusTag = (status: AssessmentStatus) => {
		const statusConfig = {
			[AssessmentStatus.Draft]: { color: 'default', text: t('assessmentDetail.status.draft') },
			[AssessmentStatus.Active]: { color: 'success', text: t('assessmentDetail.status.active') },
			[AssessmentStatus.Expired]: { color: 'warning', text: t('assessmentDetail.status.expired') },
			[AssessmentStatus.Archived]: { color: 'error', text: t('assessmentDetail.status.archived') },
		};
	...
	const handlePublish = async () => {
		...
		modal.confirm({
			...
			content: (
				<div>
					<p>
						<strong>⚠️ {t('assessmentDetail.publish.warning')}:</strong> {t('assessmentDetail.publish.warningText')}
					</p>
					...
					<ul style={{ marginLeft: 20 }}>
						...
						<li style={{ color: hasValidPoints ? 'green' : 'orange' }}>
							{hasValidPoints ? '✓' : '⚠'} {t('assessmentDetail.publish.totalPoints', { current: totalPoints })}
						</li>
						<li style={{ color: assessment.description ? 'green' : 'orange' }}>
							{assessment.description ? '✓' : '⚠'} {t('assessmentDetail.publish.hasDescription')}
						</li>
					</ul>
				</div>
			),
		...
		<Space direction="vertical" size="large" style={{ width: '100%' }}>
			...
			<Card title={t('assessmentDetail.details.title')}>
				<Descriptions column={{ xs: 1, sm: 2, lg: 3 }} bordered>
					...
					<Descriptions.Item label={t('assessmentDetail.details.questionsCount')}>
						<FileTextOutlined /> {assessment.questions_count || 0} {t('assessmentDetail.details.questionsUnit')}
					</Descriptions.Item>
			...
			{assessment.settings && (
				<Card title={t('assessmentDetail.settings.title')}>
					<Descriptions column={{ xs: 1, sm: 2, lg: 3 }} bordered>
						<Descriptions.Item label={t('assessmentDetail.settings.randomizeQuestions')}>
						...
						<Descriptions.Item label={t('assessmentDetail.settings.preventTabSwitching')}>
