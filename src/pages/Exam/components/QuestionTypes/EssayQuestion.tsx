		...
		<Space direction="vertical" style={{ width: "100%" }} size="middle">
			...
			<Card size="small" style={{ backgroundColor: "#fafafa" }}>
				<Space split={<span>|</span>}>
					<Text>
						<strong>
							{t("exam.questionTypes.essay.wordCount")}:
						</strong>{" "}
						<Tag
							color={
					...
					<Text>
						<strong>
							{t("exam.questionTypes.essay.characters")}:
						</strong>{" "}
						<Tag color="blue">{currentText.length}</Tag>
					</Text>
					{minWords && (
						<Text
							type={wordCount < minWords ? "danger" : "secondary"}
						>
							{t("exam.questionTypes.essay.minWords", {
								count: minWords,
							})}
						</Text>
					)}
					{maxWords && (
						<Text
							type={wordCount > maxWords ? "danger" : "secondary"}
						>
							{t("exam.questionTypes.essay.maxWords", {
								count: maxWords,
							})}
						</Text>
					)}
