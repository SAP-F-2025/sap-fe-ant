...
const GroupDetail: React.FC = () => {
	...
	const getRoleTag = (role: GroupMemberRole) => {
		...
		return (
			<Tag color={config[role]?.color}>
				{t(config[role]?.labelKey) || role}
			</Tag>
		);
	};
	...
	const memberColumns: ColumnsType<GroupMemberResponse> = [
		...
		},
		{
			...
			render: (_, record) => (
				<Space>
					{record.can_modify &&
						record.role !== GroupMemberRole.Owner && (
					...
					{record.can_remove && (
						<Popconfirm
							...
							description={t(
								"groups.members.removeConfirm.content",
								{
									name:
										record.user?.full_name ||
										record.user_id,
							...
							cancelText={t(
								"groups.members.removeConfirm.cancelText",
							)}
					...
					{group.can_edit && (
						<Tooltip title={t("groups.tooltip.edit")}>
							<Button
								icon={<EditOutlined />}
								onClick={() =>
									navigate(`${basePath}/${groupId}/edit`)
								}
							/>
						</Tooltip>
					)}
				...
				<Col xs={24} lg={6} xl={5}>
					...
					>
						...
						>
							...
							<Flex justify="space-between" align="center">
								<Text type="secondary">
									{t("groups.columns.memberCount")}:
								</Text>
								<Text strong>{group.member_count || 0}</Text>
							</Flex>
							<Flex justify="space-between" align="center">
								<Text type="secondary">
									{t("groups.columns.createdAt")}:
								</Text>
								<Text>
						...
						rules={[
							{
								required: true,
								message: t(
									"groups.members.addModal.selectRequired",
								),
							},
						]}
					>
						<Select
							showSearch
							placeholder={t(
								"groups.members.addModal.searchPlaceholder",
							)}
