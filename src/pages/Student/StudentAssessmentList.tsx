...
const StudentAssessmentList: React.FC = () => {
	...
	// Calculate statistics from ALL assessments
	const stats = useMemo(() => {
		return {
			total: total,
			active:
	...
	const fetchAssessments = async () => {
		setLoading(true);
		try {
			// Note: This might return all assessments.
			// Ideally backend should filter by creator_id = current user
			// or we filter here if API supports it.
			// For now assuming API returns what we need or we accept seeing all.
			const response = await assessmentService.getAssessments(filters);

			// Client-side filtering if needed (only if API returns everything and we want to restrict)
			// But pagination makes this tricky.
			// Let's assume for now we display what the API gives us.
			setAssessments(response.assessments);
			setTotal(response.total);
		} catch (error) {
			showError(
				t("assessmentList.loadError") || "Failed to load assessments",
			);
		} finally {
			setLoading(false);
		}
			...
			render: (text, record) => (
				<Space direction="vertical" size={0}>
					<Typography.Link
						strong
						onClick={() =>
							navigate(`/student/manage-assessments/${record.id}`)
						}
					>
						{text}
					</Typography.Link>
				...
				<Space size="small" style={{ display: "flex" }}>
					<Tooltip title={t("assessmentList.viewDetail")}>
						<Button
							...
							onClick={() =>
								navigate(
									`/student/manage-assessments/${record.id}`,
								)
							}
						/>
					</Tooltip>
					<Tooltip title={t("assessmentList.edit")}>
						<Button
							...
							onClick={() =>
								navigate(
									`/student/manage-assessments/${record.id}/edit`,
								)
							}
						/>
					</Tooltip>
