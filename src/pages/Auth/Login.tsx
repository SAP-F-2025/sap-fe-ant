...
import { LoginOutlined, SafetyOutlined, ThunderboltOutlined, RocketOutlined } from '@ant-design/icons';
...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      sphere.userData.velocity = { x: (Math.random() - 0.5) * 0.008, y: (Math.random() - 0.5) * 0.008 };
      spheres.push(sphere);
      scene.add(sphere);
    }
    ...
    <div style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden' }}>
      ...
      {showWelcome && (
        <div style={{
          ...
          background: 'linear-gradient(135deg, rgba(0, 204, 204, 0.95) 0%, rgba(0, 153, 230, 0.95) 100%)',
          animation: 'fadeOut 0.4s ease-out 1.6s forwards',
        }}>
          <div style={{ textAlign: 'center', animation: 'scaleIn 0.6s ease-out' }}>
            <div style={{ fontSize: 72, marginBottom: 16 }}>🎓</div>
            <Title level={1} style={{ color: 'white', fontSize: 48, margin: 0, fontWeight: 700 }}>
              Chào mừng
            </Title>
            <Text style={{ color: 'rgba(255, 255, 255, 0.9)', fontSize: 20 }}>Secure Assessment Platform</Text>
          </div>
        </div>
      )}
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', minHeight: '100vh' }}>
        ...
        <div style={{
          ...
          <Title level={1} style={{ color: 'white', fontSize: 48, marginBottom: 16, fontWeight: 700 }}>
            Secure Assessment Platform
          </Title>
          <Text style={{ color: 'rgba(255, 255, 255, 0.9)', fontSize: 18, marginBottom: 48 }}>
            Hệ thống quản lý đánh giá và kiểm tra trực tuyến hiện đại
          </Text>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            {[
              { icon: <SafetyOutlined />, text: 'Bảo mật cao với Casdoor SSO' },
              { icon: <ThunderboltOutlined />, text: 'Hiệu suất nhanh và ổn định' },
              { icon: <RocketOutlined />, text: 'Giao diện hiện đại, dễ sử dụng' },
            ].map((item, idx) => (
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: 40 }}>
              <Title level={2} style={{ color: 'white', marginBottom: 8, fontSize: 32 }}>Chào mừng trở lại</Title>
            ...
            <div style={{
              ...
              <Text style={{ color: 'rgba(255, 255, 255, 0.85)', fontSize: 13 }}>
                Sử dụng tài khoản Casdoor của bạn để đăng nhập
              </Text>
            </div>
          </div>
        </div>
