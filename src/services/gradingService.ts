...
class GradingService {
   ...
   */
  async getAttempts(params?: PaginationParams & {
    assessment_id?: number;
    student_id?: number;
    status?: string;
  ...
  async autoGradeAttempt(attemptId: number): Promise<AutoGradeAttemptResponse> {
    return apiService.post<AutoGradeAttemptResponse>(`/api/v1/grading/attempts/${attemptId}/auto`);
  }
  ...
  async getGradingOverview(assessmentId: number): Promise<GradingOverview> {
    return apiService.get<GradingOverview>(`/api/v1/grading/assessments/${assessmentId}/overview`);
  }
  ...
  async getAttemptsByStudent(
    ...
    return apiService.get<PaginatedAttemptResponse>(`/api/v1/attempts/student/${studentId}`, params);
  }
