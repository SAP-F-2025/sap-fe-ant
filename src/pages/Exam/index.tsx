...
const TakeAssessment: React.FC = () => {
	...
	const handleSubmit = async () => {
		...
		modal.confirm({
			...
			content: (
				<div>
					<p>{t('exam.answeredCount', { answered: answeredCount, total: totalQuestions })}</p>
		...
		<div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
			...
			<Card>
				<Space direction="vertical" size="large" style={{ width: '100%' }}>
					...
					<div>
						<Space>
							<Tag color="blue">{t('exam.questionN', { n: currentQuestionIndex + 1 })}</Tag>
							<Tag>{t('exam.points', { points: currentQuestion.points })}</Tag>
              ...
              */}
					...
					<Row justify="space-between" align="middle">
						...
						<Col>
							<Space>
								<Text type="secondary">
									{t('exam.answeredProgress', { answered: answeredCount, total: questions.length })}
								</Text>
							</Space>
						</Col>
						<Col>
				...
				<Card title={t('exam.violationWarning')} style={{ marginTop: '16px' }}>
					{Array.from(browserViolations.values()).map((violation) => {
						const getMessage = (type: string, metadata?: any) => {
							switch (type) {
								case 'tab_switch': return metadata?.hidden ? t('exam.violations.tabSwitch') : t('exam.violations.tabReturn');
								case 'fullscreen_exit': return t('exam.violations.fullscreenExit');
								case 'copy_paste': return metadata?.action === 'copy' ? t('exam.violations.copy') : metadata?.action === 'paste' ? t('exam.violations.paste') : t('exam.violations.cut');
								case 'browser_tamper': return t('exam.violations.devtools');
								default: return t('exam.violations.violation');
							}
						};
						return (
							<Alert
