		...
		},
		{
			...
			render: (status: string) => {
				...
				> = {
					...
					abandoned: { color: 'default', text: t('studentProgress.status.abandoned'), icon: null },
					timeout: { color: 'error', text: t('studentProgress.status.timeout'), icon: null },
				};
			...
			render: (passed: boolean | undefined, record: Attempt) =>
				record.status === 'completed' && passed !== undefined ? (
					<Tag color={passed ? 'success' : 'error'}>
						{passed ? t('studentProgress.result.passed') : t('studentProgress.result.failed')}
					</Tag>
				) : (
					<Text type="secondary">-</Text>
				),
				...
				<Space direction="vertical" style={{ width: '100%' }} size="large">
					<Space style={{ width: '100%' }} size="middle">
						...
						<Input
							...
							style={{ width: 300 }}
						/>
					</Space>					{/* Stats for selected assessment */}
				...
				<Card style={{ marginTop: '24px' }}>
					<Table
						...
						pagination={{
							...
							showTotal: (total) => t('studentProgress.totalAttempts', { count: total }),
						}}
						locale={{
