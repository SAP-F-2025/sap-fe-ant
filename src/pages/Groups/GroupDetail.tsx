...
const GroupDetail: React.FC = () => {
	...
	const getRoleTag = (role: GroupMemberRole) => {
		const config: Record<GroupMemberRole, { color: string; labelKey: string }> = {
			[GroupMemberRole.Owner]: { color: 'gold', labelKey: 'groups.role.owner' },
			[GroupMemberRole.CoOwner]: { color: 'blue', labelKey: 'groups.role.coOwner' },
			[GroupMemberRole.Member]: { color: 'default', labelKey: 'groups.role.member' },
		};
				...
				<Space>
					...
					{record.can_remove && (
						<Popconfirm
							...
							description={t('groups.members.removeConfirm.content', { name: record.user?.full_name || record.user_id })}
		...
		<Space direction="vertical" size="large" style={{ width: '100%' }}>
				...
				<Col xs={24} lg={6} xl={5}>
					<Card size="small" style={{ ...elevation[1], borderRadius: 16 }}>
						<Space direction="vertical" size={8} style={{ width: '100%' }}>
							<Text strong style={{ fontSize: 14 }}>{t('groups.detail.info')}</Text>
							...
							<Flex justify="space-between" align="center">
								...
								<Tag color={group.type === 'class' ? 'purple' : 'cyan'} style={{ margin: 0 }}>
									{group.type === 'class' ? t('groups.type.class') : t('groups.type.studyGroup')}
								</Tag>
							</Flex>
							<Flex justify="space-between" align="center">
			...
			>
				<Form form={form} layout="vertical" onFinish={handleAddMember}>
					<Form.Item
						...
						rules={[{ required: true, message: t('groups.members.addModal.selectRequired') }]}
					>
						<Select
							showSearch
					...
					>
						<Select
							options={[
								{ label: t('groups.role.member'), value: GroupMemberRole.Member },
								{ label: t('groups.role.coOwner'), value: GroupMemberRole.CoOwner },
							]}
						/>
					</Form.Item>
			...
			>
				<Form form={roleForm} layout="vertical" onFinish={handleChangeRole}>
					...
					<Form.Item style={{ marginBottom: 0, textAlign: 'right' }}>
						<Space>
							<Button onClick={() => setRoleModalOpen(false)}>{t('common.cancel')}</Button>
