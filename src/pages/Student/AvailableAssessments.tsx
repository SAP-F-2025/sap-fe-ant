		...
		try {
					...
					onOk: async () => {
						try {
							...
							modal.error({
								...
								content: error.message || t('availableAssessments.modals.error.cannotContinue'),
							});
						}
					},
				});
			...
			if (!canStartResult.can_start) {
				modal.error({
					...
					content: canStartResult.message || t('availableAssessments.modals.cannotStart.defaultContent'),
				});
				return;
			}
		...
		modal.confirm({
			...
			content: (
				<div>
					...
					<p>{t('availableAssessments.modals.startAssessment.duration', { duration: selectedAssessment.duration })}</p>
					<p>{t('availableAssessments.modals.startAssessment.attempts', { used: selectedAssessment.attempts_used, max: selectedAssessment.max_attempts })}</p>
					<p>{t('availableAssessments.modals.startAssessment.passingScore', { score: selectedAssessment.passing_score })}</p>
					<Alert
			...
			onOk: async () => {
				try {
					...
					modal.error({
						title: t('availableAssessments.modals.error.title'),
						content: error.response?.data?.message || error.message || t('availableAssessments.modals.error.cannotStart'),
					});
				}
			},
		});
	...
	const columns = [
		...
		},
		{
			...
			render: (date: string | undefined) => {
				if (!date) return <Text type="secondary">{t('availableAssessments.noDueDate')}</Text>;
		...
		},
		{
			...
			render: (_: any, record: StudentAssessment) => {
				if (record.has_active_attempt) {
					return <Tag color="processing">{t('availableAssessments.status.inProgress')}</Tag>;
				}
				if (!record.can_start) {
					...
					<Space direction="vertical" size="large" style={{ width: '100%' }}>
						...
						<Table
							...
							pagination={{
								...
								showTotal: (total) => t('availableAssessments.totalAssessments', { count: total }),
